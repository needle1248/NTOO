...
document.getElementById("navigationForm").addEventListener("submit", async (event) => {
  ...
  const submitToken = issueStateRenderToken();
  ...
  try {
    ...
    if (submitToken === latestStateRenderToken && data.navigation) {
      renderNavigationPanel(data.navigation);
      maybeSpeakNavigation(data.navigation);
    }
  } catch (error) {
...
setInterval(() => refreshState({ source: "interval" }).catch(() => null), 2500);
